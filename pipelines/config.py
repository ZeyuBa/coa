...
def _init_pipeline_graph(root_node_name: Optional[str],is_plot=False) -> nx.DiGraph:
    """
...
def _add_node_to_pipeline_graph(
    ...
    try:
        for input_node in node["inputs"]:
            ...
            #  graph.add_edge(input_node_name, node["name"], label=node['edges'][input_node_name])
            if bidirection_list:
                print(1)
                bidirectional_match = any(input_node_name in b_l and node["name"] in b_l for b_l in bidirection_list)
                
                if bidirectional_match:
                    graph.add_edge(input_node_name, node["name"], label=node['edges'][input_node_name][0])
                    graph.add_edge(node["name"], input_node_name, label='feed_back')
                else:
                    graph.add_edge(input_node_name, node["name"], label=node['edges'][input_node_name][0])
