...
class Pipeline:
    ...
    def __init__(self,is_plot=False,bidirection_list=[]):
        self.graph = DiGraph()
        self.config_hash = None
        self.last_config_hash = None
        self.runs = 0
        self.is_plot=is_plot
        self.bidirection_list=bidirection_list
    ...
    def add_node(self, component: BaseComponent, name: str, inputs: List[str],input_edges:Dict[str,str]):
        """
        ...
        :param input_name_to_edges: Dict map from input_node to input_edges.
        """
        ...
        if len(self.graph.nodes) < 1:
            ...
            self.graph = _init_pipeline_graph(root_node_name=candidate_roots[0],is_plot=self.is_plot)
        ...
        self.graph = _add_node_to_pipeline_graph(
            ...
            node={"name": name, "inputs": inputs,"edges":input_edges},
            instance=component,
            bidirection_list=self.bidirection_list
        )
        ...
        while queue:
            ...
            if predecessors.isdisjoint(set(queue.keys())):  # only execute if predecessor nodes are executed
                try:
                    ...
                    node_output, stream_id =  self._arun_node(node_id, node_input)
                    if "_debug" in node_output and node_id in node_output["_debug"]:
                        node_output["_debug"][node_id]["exec_time_ms"] = round((time() - start) * 1000, 2)
                except Exception as e:
                    # The input might be a really large object with thousands of embeddings.
